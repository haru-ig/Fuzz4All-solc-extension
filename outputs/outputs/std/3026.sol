...
contract MutatedArrayNonSemanticEquivalent {
    uint[] private _array;
    ...
    function addTwoTwicesToLength() public {
        uint[] memory _numArray = new uint[](_array.length*2);
        for (uint ii = 0; ii < _array.length; ii++) {
            _numArray[ii*2] = _array[ii];
            _numArray[ii*2+1] = _array[ii];
        }
        _array = _numArray;
    }
