...
contract ExampleClass {
    ...
    function getArrAt(uint index) external view returns (uint) {
        return index > _arraySize-1? _arraySize-1 : index;
    }
    function getMin(uint[2] memory inputArr) internal view returns (uint) {
    ...
    function getMin3(uint[3] memory inputArr) internal view returns (uint) {
        uint minValue = inputArr[0];
        for (uint i=1; i<inputArr.length; i++) {
            if(inputArr[i]< minValue) minValue = inputArr[i];
        }
