...
contract ArrayMutationsNoMutations {
    ...
    function setArrayToMutated() public {
        uint[] memory _previousArray = _array;
        for (uint ii = 0; ii < _previousArray.length; ii++) {
            uint previousArrayIndex = ii;
        }
        _lastNum = _previousArray;
        _array = _previousArray;
    }
